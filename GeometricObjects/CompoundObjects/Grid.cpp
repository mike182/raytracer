...
// not implemented
Grid::Grid([[maybe_unused]]const Grid& grid) {
}

// not implemented
Grid& Grid::operator=([[maybe_unused]]const Grid& rhs) {
    return *this;
}
...
void Grid::read_ply_file(char* file_name, const int triangle_type) {
    ...
    PlyProperty** plist;
    [[maybe_unused]]Vertex** vlist;
    [[maybe_unused]]Face** flist;
    char* elem_name;
    int num_comments;
    char** comments;
    int num_obj_info;
...
void Grid::compute_mesh_normals(void) {
    ...
    for (int index = 0; index < mesh_ptr->num_vertices; index++) {   // for each vertex
        ...
        for (std::size_t j = 0; j < mesh_ptr->vertex_faces[index].size(); j++)
